...
function DashboardContent() {
  ...
  const dayBlocks = useMemo(
    () => applyFilter(getBlocksForDate(selectedDate, recurringBlocks, exceptions)),
    [selectedDate, recurringBlocks, exceptions, categoryFilter]
  );

  const weekBlocksByDate = useMemo(() => {
    const map = getBlocksForRange(weekDates, recurringBlocks, exceptions);
    for (const [key, blocks] of map) map.set(key, applyFilter(blocks));
    return map;
  }, [weekDates, recurringBlocks, exceptions, categoryFilter]);

  const monthBlocksByDate = useMemo(() => {
    const map = getBlocksForRange(monthDates, recurringBlocks, exceptions);
    for (const [key, blocks] of map) map.set(key, applyFilter(blocks));
    return map;
  }, [monthDates, recurringBlocks, exceptions, categoryFilter]);
