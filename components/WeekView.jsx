...
export default function WeekView({ weekDates, today, blocksByDate, onBlockClick, onDayClick, onAddClick }) {
  return (
    <div className="grid grid-cols-1 gap-3 sm:grid-cols-7">
      {weekDates.map((date) => {
        ...
        return (
          <div key={iso} className="rounded-lg border border-line bg-surface/60">
            <button
              type="button"
            ...
            <div className="space-y-1.5 p-2">
              {blocks.length === 0 ? (
                ...
                blocks.map((block) => (
                  <BlockCard key={block.key} block={block} onClick={onBlockClick} compact />
                ))
              )}
              <button
                type="button"
